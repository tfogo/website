...
1. SSH into `etcd0` and run the following:
    ...
    {{< note >}}**Optional:** You can modify `ca-csr.json` to add a section for `names`. See [the CFSSL wiki](https://github.com/cloudflare/cfssl/wiki/Creating-a-new-CSR) for an example.{{< /note >}}
...
1. Create the following configuration file _/etc/keepalived/keepalived.conf_ on all master nodes:
...
1. Install the following health check script to _/etc/keepalived/check_apiserver.sh_ on all master nodes:
...
1. Restart keepalived. While no Kubernetes services are up yet it will log health check fails on all master nodes. This will stop as soon as the first master node has been bootstrapped.
{{% /tab %}}
{{< /tabs >}}
...
1. In order for kubeadm to run, you first need to write a configuration file:
        ...
        <!-- Not using code fencing because of the bug russross/blackfriday#239 -->
        cat >config.yaml <<EOF
